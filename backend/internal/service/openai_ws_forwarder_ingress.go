...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	for {
		...
		result, relayErr := sendAndRelay(turn, sessionLease, currentPayload, currentPayloadBytes, currentOriginalModel, currentRoutingModel, currentImageBillingModel, currentImageSizeTier, currentImageInputSize, currentRequestedReasoningEffort)
		if relayErr != nil {
			lastTurnClean = false
