...
// Accounts Management
    accounts: {
      ...
      openai: {
        ...
        codexCLIOnlyDesc: '适用于 OpenAI OAuth 与 API Key。开启后仅允许 Codex 官方客户端家族访问；关闭后完全绕过并保持原逻辑。',
        clientPolicy: '客户端访问策略',
        clientPolicyDesc: '适用于 OpenAI OAuth 与 API Key。可允许任意客户端、限制为 Codex 客户端，或要求请求命中 TLS 路由器规则。',
        clientPolicyAny: '允许任意客户端',
        clientPolicyCodexOnly: '只允许 Codex 客户端',
        clientPolicyTLSRouterMatchedOnly: '只允许 TLS 路由器匹配到的客户端',
        clientPolicyTLSRouterRequired: '选择 TLS 路由器匹配策略时，必须启用 TLS 指纹并选择 TLS 路由器。',
