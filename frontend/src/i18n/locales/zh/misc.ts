...
// Payment System
  payment: {
    ...
    rechargeRatePreview: '当前倍率：1 {currency} = {amount} {unitName}',
    refundReason: '退款原因',
    ...
    errors: {
      ...
      PAYMENT_STATUS_UNAVAILABLE: '暂时无法确认支付状态，请稍后重试或联系管理员处理',
      PAYMENT_PROVIDER_TEST_INVALID_CONFIG: '支付服务商测试配置无效',
      PAYMENT_PROVIDER_TEST_FAILED: '支付服务商连通性测试失败',
      UNSUPPORTED_PROVIDER_TEST: '当前支付服务商不支持连通性测试',
      PROVIDER_NOT_FOUND: '支付服务商实例不存在',
      PROVIDER_KEY_MISMATCH: '支付服务商类型与实例不匹配',
      ...
      FORCED_EXPIRED_ORDERS: '该服务商仍有等待迟到付款恢复的强制过期订单，不能删除；请停用并保留该服务商。',
      PAYMENT_PROVIDER_CONFLICT: '该支付方式已有其他启用中的服务商实例，请先停用后再继续。',
      CANCEL_RATE_LIMITED: '取消订单过于频繁，请稍后再试',
      INVALID_FORCE_EXPIRE_REASON: '强制过期原因不能为空且不能超过 500 个字符',
      ORDER_STATUS_CHANGED: '订单状态已变更，列表已刷新',
      NOT_FOUND: '订单不存在',
      FORBIDDEN: '无权限操作此订单',
    ...
    admin: {
      ...
      forceExpire: '强制过期',
      forceExpireOrder: '强制使订单过期',
      forceExpireWarning: '该操作不会查询或关闭上游支付。若渠道稍后确认付款，订单仍会恢复并履约。',
      forceExpireReason: '强制过期原因',
      forceExpireReasonPlaceholder: '例如：支付服务商地址配置错误，无法查询订单状态',
      forceExpireAcknowledge: '我确认该订单的上游支付状态当前无法确认。',
      confirmForceExpire: '确认强制过期',
      forceExpireSuccess: '订单已强制过期',
