...
marketplace: {
    ...
    collapsePricing: 'Hide detailed pricing',
    ...
    fastCacheWrite1h: 'Fast Cache Write (1h)',
...
// Payment System
  payment: {
    ...
    rechargeRatePreview: 'Current rate: 1 {currency} = {amount} {unitName}',
    ...
    errors: {
      ...
      PAYMENT_STATUS_UNAVAILABLE: 'Payment status cannot be confirmed right now. Please retry later or contact an administrator.',
      PAYMENT_PROVIDER_TEST_INVALID_CONFIG: 'Payment provider test configuration is invalid',
      PAYMENT_PROVIDER_TEST_FAILED: 'Payment provider connectivity test failed',
      UNSUPPORTED_PROVIDER_TEST: 'This payment provider does not support connectivity tests',
      PROVIDER_NOT_FOUND: 'Payment provider instance was not found',
      PROVIDER_KEY_MISMATCH: 'Payment provider type does not match the instance',
      ...
      FORCED_EXPIRED_ORDERS: 'This provider has force-expired orders waiting for late-payment recovery and cannot be deleted. Disable and retain it instead.',
      ...
      INVALID_FORCE_EXPIRE_REASON: 'Force-expiration reason is required and must not exceed 500 characters.',
      ORDER_STATUS_CHANGED: 'Order status changed. The list has been refreshed.',
    ...
    admin: {
      ...
      forceExpireOrder: 'Force Expire Order',
      forceExpireWarning: 'This does not query or close the upstream payment. If the provider later confirms payment, the order will still recover and be fulfilled.',
      forceExpireReason: 'Force-expiration reason',
      forceExpireReasonPlaceholder: 'For example: the provider URL is misconfigured and the order cannot be queried',
      forceExpireAcknowledge: 'I confirm that the upstream payment status cannot currently be verified.',
      confirmForceExpire: 'Confirm Force Expiration',
      forceExpireSuccess: 'Order was force-expired',
